...
export class VerityState{
  constructor(seed=1){
    ...
    this.eastVillage={generated:false,x:0,z:0,towerX:0,towerZ:0,shrineX:0,shrineY:null,shrineZ:0,visited:false};
  ...
  apply(action,amount=1,meta={}){const n=Math.max(0,Number(amount)||0);switch(action){case'praise':this.bond+=3*n;this.resentment-=n;this.metrics.praises+=n;break;case'insult':this.resentment+=4*n;this.bond-=2*n;this.metrics.insults+=n;break;case'ask':this.curiosity+=.8*n;this.dependency+=.4*n;this.metrics.questions+=n;break;case'talk':this.bond+=.25*n;this.dependency+=.2*n;this.metrics.conversations+=n;break;case'abandon':this.resentment+=2.2*n;this.dependency+=1.5*n;this.metrics.abandons+=n;break;case'help':this.bond+=2*n;this.resentment-=.5*n;break;case'hit':this.resentment+=8*n;this.fear+=5*n;this.bond-=6*n;break;case'investigate':this.curiosity+=2*n;this.corruption+=1.2*n;break;case'die':this.fear+=4*n;this.dependency+=2*n;this.metrics.deathsWitnessed+=n;break;case'repel':this.fear+=3*n;this.resentment+=n;this.demon.repelled+=n;break;}this.bond=clamp(this.bond,0,100);this.resentment=clamp(this.resentment,0,100);this.dependency=clamp(this.dependency,0,100);this.curiosity=clamp(this.curiosity,0,100);this.fear=clamp(this.fear,0,100);this.corruption=clamp(this.corruption,0,100);this.updatedAt=Date.now();if(meta?.text)this.memory.remember(action,meta.text,Math.max(.5,n),meta);}
  tick(dt,{distance=0,near=true,intensity=1}={}){if(!this.box.opened||this.ending)return;const seconds=Math.max(0,Number(dt)||0),rate=clamp(Number(intensity)||1,.4,1.6);this.exposure+=seconds*rate*(1+this.resentment*.0025+this.corruption*.002);if(near){this.metrics.secondsTogether+=seconds;this.bond=clamp(this.bond+seconds*.002,0,100);}else{this.metrics.secondsFar+=seconds;if(distance>35)this.dependency=clamp(this.dependency+seconds*.003,0,100);}const corruptionRate=this.phase>=VERITY_PHASES.UNCANNY?.0025:.00035;this.corruption=clamp(this.corruption+seconds*corruptionRate*rate,0,100);this.updatedAt=Date.now();this.evaluatePhase();}
