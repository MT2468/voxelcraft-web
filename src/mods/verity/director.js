...
export class HorrorDirector{
  constructor(state,{random=Math.random,onEvent=()=>{}}={}){this.state=state;this.random=random;this.onEvent=onEvent;this.clock=0;this.lastPlayer=null;}
  tick(dt,ctx){if(!this.state.box.opened||this.state.ending)return null;this.clock+=dt;const d=this.state.director;d.nextEventIn-=dt;if(d.nextEventIn>0)return null;const candidates=EVENTS.filter((e)=>this.eligible(e,ctx));if(!candidates.length){d.nextEventIn=20;return null;}const weighted=[];for(const e of candidates)for(let i=0;i<e.weight;i++)weighted.push(e);const event=weighted[Math.floor(this.random()*weighted.length)];this.fire(event,ctx);d.lastEventAt=Date.now();d.nextEventIn=this.nextDelay();d.eventCounts[event.id]=(d.eventCounts[event.id]||0)+1;d.recent.push(event.id);if(d.recent.length>6)d.recent.shift();return event.id;}
  eligible(e,ctx){const s=this.state,d=s.director;if(s.phase<e.min||s.phase>(e.max??99))return false;if((d.eventCounts[e.id]||0)>=(e.maxCount??999))return false;if(d.recent.includes(e.id))return false;if(e.test&&!e.test(ctx,s))return false;return true;}
  fire(event,ctx){const s=this.state,api=ctx.api;switch(event.id){
    ...
    case'mark':placeVerityMark(api.world,Math.floor(ctx.position.x+randOffset(this.random,8,18)),Math.floor(ctx.position.z+randOffset(this.random,8,18)));break;
    case'corruption':corruptPatch(api.world,Math.floor(ctx.position.x+randOffset(this.random,10,24)),Math.floor(ctx.position.z+randOffset(this.random,10,24)),{radius:3+Math.floor(this.random()*3),intensity:.28+.25*this.random()});break;
    case'vanish':s.verity.visible=false;setTimeout(()=>{s.verity.visible=true;s.verity.x=ctx.position.x-3;s.verity.z=ctx.position.z-3;},3500+this.random()*3500);break;
  ...
  nextDelay(){const p=this.state.phase;const base=p<=VERITY_PHASES.FRIEND?55:p===VERITY_PHASES.ATTACHED?42:p===VERITY_PHASES.UNCANNY?31:p===VERITY_PHASES.STALKING?22:16;return base*(.72+this.random()*.65);}
}
...
const EVENTS=[
  ...
  {id:'blackout',min:VERITY_PHASES.UNCANNY,weight:3},
  {id:'mark',min:VERITY_PHASES.UNCANNY,weight:2,maxCount:8},
  ...
  {id:'corruption',min:VERITY_PHASES.STALKING,weight:3,maxCount:12},
  {id:'night_lock',min:VERITY_PHASES.STALKING,max:VERITY_PHASES.STALKING,weight:2,maxCount:3,test:(ctx)=>ctx.api.world.dimension==='overworld'},
  ...
  {id:'blackout',min:VERITY_PHASES.DEMON,weight:5},
  {id:'corruption',min:VERITY_PHASES.DEMON,weight:4,maxCount:20}
];
