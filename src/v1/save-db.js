const DB_NAME='voxelcraft-worlds';const DB_VERSION=1;const STORES=['worlds','chunks','players','settings'];
...
export class SaveDB {
  ...
  async put(store,key,value){await this.open();if(!this.db){this.memory.set(`${store}:${key}`,structuredCloneSafe(value));return true;}return txPromise(this.db,store,'readwrite',(s)=>s.put(value,key));}
  async get(store,key){await this.open();if(!this.db)return structuredCloneSafe(this.memory.get(`${store}:${key}`));return txPromise(this.db,store,'readonly',(s)=>s.get(key));}
  async delete(store,key){await this.open();if(!this.db){this.memory.delete(`${store}:${key}`);return true;}return txPromise(this.db,store,'readwrite',(s)=>s.delete(key));}
  async keys(store){await this.open();if(!this.db)return[...this.memory.keys()].filter((k)=>k.startsWith(`${store}:`)).map((k)=>k.slice(store.length+1));return txPromise(this.db,store,'readonly',(s)=>s.getAllKeys());}
  async saveWorld(worldId,payload){const now=Date.now(),meta={id:worldId,name:payload.meta?.name||worldId,seed:payload.world?.seed,dimension:payload.world?.dimension||'overworld',gameMode:payload.player?.stats?.gameMode||'survival',updatedAt:now,createdAt:payload.meta?.createdAt||now,version:1};await this.put('worlds',worldId,{meta,world:payload.world,automation:payload.automation,advancements:payload.advancements,rules:payload.rules});await this.put('players',worldId,payload.player);return meta;}
  async loadWorld(worldId){const root=await this.get('worlds',worldId);if(!root)return null;const player=await this.get('players',worldId);return{...root,player};}
  ...
  async deleteWorld(worldId){await this.delete('worlds',worldId);await this.delete('players',worldId);const chunkKeys=await this.keys('chunks');for(const key of chunkKeys)if(String(key).startsWith(`${worldId}:`))await this.delete('chunks',key);}
