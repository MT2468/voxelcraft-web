...
export class WorldRendererV1{
  constructor(scene,world,{renderRadius=3}={}){this.scene=scene;this.world=world;this.renderRadius=renderRadius;this.meshes=new Map();this.atlas=createTextureAtlas(THREE,BLOCKS);this.solidMaterial=new THREE.MeshLambertMaterial({map:this.atlas.texture,vertexColors:true,alphaTest:.18});this.liquidMaterial=new THREE.MeshLambertMaterial({map:this.atlas.texture,vertexColors:true,transparent:true,opacity:.7,depthWrite:false,side:THREE.DoubleSide});this.rebuildBudget=2;}
  dispose(){for(const pair of this.meshes.values())for(const mesh of Object.values(pair))if(mesh){this.scene.remove(mesh);mesh.geometry.dispose();}this.solidMaterial.dispose();this.liquidMaterial.dispose();this.atlas.texture.dispose();this.meshes.clear();}
  update(centerX,centerZ){const ccx=Math.floor(centerX/16),ccz=Math.floor(centerZ/16);this.world.ensureAround(centerX,centerZ,this.renderRadius);for(const[key,pair]of this.meshes)if(!this.world.chunks.has(key)){for(const mesh of Object.values(pair))if(mesh){this.scene.remove(mesh);mesh.geometry.dispose();}this.meshes.delete(key);}
    const dirty=[...this.world.chunks.values()].filter((c)=>c.dirty&&Math.abs(c.cx-ccx)<=this.renderRadius&&Math.abs(c.cz-ccz)<=this.renderRadius).sort((a,b)=>(Math.abs(a.cx-ccx)+Math.abs(a.cz-ccz))-(Math.abs(b.cx-ccx)+Math.abs(b.cz-ccz)));
...
function geometryForChunk(world,chunk,atlas,liquidPass){const pos=[],norm=[],uv=[],colors=[],ind=[];let base=0;const sx=chunk.cx*16,sz=chunk.cz*16,color=new THREE.Color();
  for(let lx=0;lx<16;lx++)for(let lz=0;lz<16;lz++)for(let y=0;y<WORLD_HEIGHT;y++){const id=chunk.get(lx,y,lz);if(!id||isLiquid(id)!==liquidPass)continue;const def=blockDef(id);if(!def)continue;const x=sx+lx,z=sz+lz;
    for(const face of FACES){const[dx,dy,dz]=face.d,nid=world.getBlock(x+dx,y+dy,z+dz);if(liquidPass){if(nid===id)continue;if(isLiquid(nid)&&nid!==id)continue;}else{if(nid&&isOpaque(nid))continue;if(nid===id&&!def.transparent)continue;}
      let light=1;if(world.dimension==='overworld'){const sky=world.skyLightAt(x+dx,y+dy,z+dz)/15;light=.18+.82*sky;}else light=world.dimension==='emberdeep'?.42:.55;const emits=def.emits||0;if(emits)light=Math.max(light,.45+emits/30);color.setHex(def.color||0xffffff).multiplyScalar(face.shade*light);
